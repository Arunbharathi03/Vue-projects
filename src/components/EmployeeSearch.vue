    ...
    <div>
        <table class="mx-auto border-solid border-2 border-black">
            ...
            <tr v-for="employee in filteredEmployees" :key="employee.id">
                <td class="w-96 py-2 border-solid border-2 border-black text-center text-blue-600 font-bold"><router-link :to="{ name: 'employeePreview', params: { itemId: employee.id }}">{{ employee.name }}</router-link></td>
                ...
                <td class="w-96 py-2 border-solid border-2 border-black text-center">
                    <div class="flex justify-around">
                        ...
                        <div class="delete cursor-pointer text-red-600" @click="local_deleteEmployee(employee.id, employee.name)">
                ...
                <div class="c-employee-edit" v-if="editMode">
                    <div class="flex flex-col items-center relative">
                        ...
                        <div class="w-96">
                            <input ref="editName" class="p-2 mb-6 w-full" type="text" placeholder="Edit name" v-model="editedName" @keyup.enter="local_focusAgeInput" maxlength="40">
                            <input ref="inputAge" min="18" max="99" maxlength="2" class="p-2 mb-6 w-full" type="number" placeholder="Edit age" v-model="editedAge" @keyup.enter="local_focusSalaryInput">
    ...
    methods: {
        local_deleteEmployee (itemId, name) {
            const index = this.employeeList.findIndex(employee => {
                return employee.id === itemId
            })
            const confirmDelete = confirm(`Delete ${name}`)
            if(confirmDelete) this.delete_employee(index)
        },
        local_editEmployee (employeeItem) {
            this.editMode = true 
            const index = this.employeeList.findIndex(employee => employee.id === employeeItem.id)
            this.edit_employee(index)

            this.editedName = employeeItem.name
            this.editedAge = employeeItem.age
            this.editedSalary = employeeItem.salary
